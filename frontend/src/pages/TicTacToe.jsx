...
export default function TicTacToe() {
  ...
  const challenge = async (opponentId) => {
    ...
    try {
      const { sessionId } = await api.challenge(opponentId, 'tictactoe');
      nav(`/game/${sessionId}`);
    } catch (e) {
      alert(e.message);
    }
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-paper-600 text-sm">Выбери соперника из онлайна</p>
      </div>

      <div className="card divide-y divide-paper-200">
        {others.length === 0 && (
          <p className="p-6 text-center text-paper-500 text-sm">Никого нет онлайн. Позови друга на сайт!</p>
        )}
        {others.map((o) => (
